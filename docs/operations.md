...
Only names and purposes belong in this reference. Dashboard variable names/scopes and secret presence were not enumerated. The MCP endpoint requires its shared rate-limit credentials in every Vercel environment and returns 503 rather than running without global enforcement.
...
| `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`               | Shared atomic MCP request counter                                                             | Required secrets for every deployed Vercel environment; provision from the project's Redis integration and rotate together                                                            |
| `MCP_RATE_LIMIT_KEY_SECRET`                                        | HMAC key that pseudonymizes the platform-verified caller address before storage               | Required secret for every deployed Vercel environment; use one stable high-entropy value across instances, never expose it to the browser                                             |
...
For MCP rate-limit verification, confirm all three secret names are present in the intended Vercel environment, then make a single ordinary MCP request and inspect existing runtime/store telemetry. A 503 with `Retry-After` means enforcement failed closed; restore the store or its credentials instead of bypassing the limiter. Never run the rate-limit regression suite against production.
